...
// ==================== AUTH-PROTECTED ROUTES (For Approved Users) ====================
Route::middleware(['auth', 'verified'])->group(function () {
    
    // Dashboard Route (Main entry point after login)
    Route::get('/dashboard', [DashboardController::class, 'index'])->name('dashboard');
    ...
    Route::resource('posts', PostController::class);
    Route::get('/my-posts', [PostController::class, 'myPosts'])->name('posts.my-posts');
    Route::post('/posts/{id}/complain', [PostController::class, 'complain'])->name('posts.complain');


    // AJAX Routes for Comments and Reports Data (Publicly accessible)
    Route::get('/posts/{id}/comments-data', [PostController::class, 'getCommentsData'])->name('posts.comments.data');
    Route::get('/posts/{id}/reports-data', [PostController::class, 'getReportsData'])->name('posts.reports.data');
...
Route::prefix('admin')->name('admin.')->middleware(['auth', 'admin'])->group(function () {
    ...
    Route::get('/statistics', function () {
        ...
        // Post statistics
        $postStats = \App\Models\Post::selectRaw('
            COUNT(*) as total,
            SUM(CASE WHEN type = "information" THEN 1 ELSE 0 END) as information,
            SUM(CASE WHEN type = "financial" THEN 1 ELSE 0 END) as financial,
            SUM(CASE WHEN type = "blood" THEN 1 ELSE 0 END) as blood,
            SUM(CASE WHEN type = "accommodation" THEN 1 ELSE 0 END) as accommodation,
            SUM(CASE WHEN type = "tuition" THEN 1 ELSE 0 END) as tuition,
